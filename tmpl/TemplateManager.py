...
class MetaInformation:
    ...
    def __init__(self, template_name: pathlib.Path):
        ...
        if metafile.is_file():
            with metafile.open(encoding="utf-8") as file:
                ...
                if "variables" in self.data:
                    ...
                    for var in self.data["variables"]:
                        ...
                        readablename = (
                            var["readablename"] if "readablename" in var else None
                        )
                        description = (
                            var["description"] if "description" in var else None
                        )
            ...
            logger.warning(
                "Could not find {}, no metainformation for {} available!".format(
                    metafile, template_name
                )
            )
            raise TemplateNotValidError(
                "Template ``{}`` has no metainformation sidecar-file.".format(metafile)
            )

    def getVarInfo(self, varname: str):
        if self.variables[varname]:
            return (
                self.variables[varname]["readablename"],
        ...
        for template_name in templates:
            try:
                ...
                for var in vars:
                    ...
                    if metainfo:
                        readablename, datatype, description, source = (
                            metainfo.getVarInfo(var)
                        )
                        variables.append(
                            dict(
                                variable=var,
                ...
                logger.error(
                    "Can not load template from filesystem: {}".format(template_name)
                )
